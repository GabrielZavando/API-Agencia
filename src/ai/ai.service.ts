...
export class AIService {
  ...
  async generateProspectResponse(
    ...
    try {
      ...
      if (providerType !== this.defaultProvider) {
        ...
        return this.generateProspectResponse(contactDto, existingProspect, this.defaultProvider);
      }
  ...
  private getDefaultProvider(): AIProviderType {
    ...
    if (defaultProvider && Object.values(AIProviderType).includes(defaultProvider)) {
  ...
  private buildAIContext(contactDto: ContactDto, existingProspect?: ProspectRecord): AIContext {
    const companyInfo: CompanyInfo = {
      name: process.env.COMPANY_NAME || 'Tu Empresa',
      description: process.env.COMPANY_DESCRIPTION || 'Empresa líder en soluciones innovadoras',
      services: (process.env.COMPANY_SERVICES || 'consultoría,desarrollo,soporte').split(','),
      values: (process.env.COMPANY_VALUES || 'calidad,innovación,confianza').split(','),
    ...
    return {
      ...
      previousConversations: this.extractConversationSummaries(existingProspect),
      companyInfo,
    };
  }

  private buildPrompt(contactDto: ContactDto, existingProspect?: ProspectRecord): string {
    let prompt = `Consulta: "${contactDto.message}"`;
    
    if (existingProspect && existingProspect.conversations.length > 0) {
      const lastConversation = existingProspect.conversations[existingProspect.conversations.length - 1];
  ...
  private extractTopic(message: string): string {
    ...
    const keywords = ['precio', 'servicio', 'consulta', 'información', 'cotización', 'soporte'];
  ...
  private getGenericResponse(contactDto: ContactDto, isReturning: boolean): string {
  ...
  async testProvider(providerType: AIProviderType, testMessage: string): Promise<AIResponse> {
    ...
    return this.generateProspectResponse(mockContactDto, undefined, providerType);
  }
}
